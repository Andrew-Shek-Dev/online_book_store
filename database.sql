...
create table cart (id serial primary key, cart_id integer unique, member_id integer, product_id integer, payment boolean default false, foreign key(member_id) references members(user_id), foreign key(product_id) references product(id));

create table invoice (id serial primary key,cart_id integer,invoice varchar(225), foreign key(cart_id) references cart(cart_id));
